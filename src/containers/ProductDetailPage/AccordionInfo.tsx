          ...
          <Disclosure key={index} defaultOpen={index < 1}>
            {({ open }) => (
              <>
                <Disclosure.Button
                  className={`${
                    open ? "bg-[#7f876f]" : "bg-[#c7af97]"
                  } hover:bg-[#7f876f] text-white flex items-center justify-between w-full px-4 py-2 font-medium text-left rounded-lg focus:outline-none focus-visible:ring focus-visible:ring-slate-500 focus-visible:ring-opacity-75 `}
                >
                  <span>{item.name}</span>
                  {!open ? (
                    <PlusIcon className="w-4 h-4 text-white-600 dark:text-slate-400" />
                  ) : (
                    <MinusIcon className="w-4 h-4 text-white-600 dark:text-slate-400" />
                  )}
                </Disclosure.Button>
                <Disclosure.Panel
